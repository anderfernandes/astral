# Astral Beta Dockerfile
# Author: @anderfernandes

# Pull archlinux image
...
# RUN pacman -S nginx-mainline php-fpm --noconfirm

# Enabling PHP extensions
RUN echo "extension=pdo_sqlite" >> /etc/php/php.ini
RUN echo "extension=sqlite3" >> /etc/php/php.ini

# Creating Astral's directory (without NGINX)
RUN mkdir /usr/share/astral

# Creating Astral's directory (with NGINX)
# RUN mkdir /usr/share/nginx/astral

# Setting Astral's workdir (without NGINX)
WORKDIR /usr/share/astral
...
# RUN echo "DB_DATABASE=/usr/share/astral/database/database.sqlite" >> .env
