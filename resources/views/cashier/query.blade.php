      ...
      <tr>
        ...
        <td class="selectable">
          @if ($result->payment_method == 'visa')
            <a href="{{ route ('admin.sales.show', $result->id) }}" target="_blank"><h2 class="ui center aligned header"><i class="visa icon"></i></h2></a>
          @elseif ($result->payment_method == 'mastercard')
            <a href="{{ route ('admin.sales.show', $result->id) }}" target="_blank"><h2 class="ui center aligned header"><i class="mastercard icon"></i></h2></a>
        ...
        <td class="selectable">
          <a href="{{ route ('admin.sales.show', $result->id) }}" target="_blank">{{ $result->reference }}</a>
        </td>
        <td  class="selectable">
          <a href="{{ route ('admin.sales.show', $result->id) }}" target="_blank">$ {{ number_format($result->total, 2) }}</a>
        </td>
        <td class="selectable"><a href="{{ route ('admin.sales.show', $result->id) }}" target="_blank">{{ $result->source }}</a></td>
        <td class="selectable"><a href="{{ route ('admin.sales.show', $result->id) }}" target="_blank">{{ App\User::find($result->cashier_id)->firstname }} {{ App\User::find($result->cashier_id)->lastname }}</a></td>
        <td class="selectable"><a href="{{ route ('admin.sales.show', $result->id) }}" target="_blank">{{ Date::parse($result->created_at)->format('l, F j, Y \a\t g:i A') }}</a></td>
      </tr>
    @endforeach
    </tbody>
  </table>
