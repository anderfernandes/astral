...
<script>
  export default {
    computed: {
      alert: {
        set(value) {
          this.$store.commit('SET_ALERT', value)
        },
        get() {
      ...
      open: {
        set(value) {
          ...
          if (value == false) this.alert = null
        },
        get() {
          return this.$store.getters.show_alert
        }
      },
...
<style>
  ...
  .list-leave-active,
  .list-move {
    transition: 0.25s cubic-bezier(0.6, 0.12, 0.34, 0.95);
  ...
  .ui.sale.segment .header,
  .ui.sale.segment .meta {
    color: white !important;
  }
