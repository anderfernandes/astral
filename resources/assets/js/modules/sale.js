...
export default {
  ...
  mutations: {
    ...
    SET_SELECTED_PRODUCTS(state, payload) {
      let selected_products = state.product_options.filter(product_option =>
        payload.includes(product_option.id)
      )
    ...
    REMOVE_PRODUCT(state, payload) {
      ...
      let j = state.sale.selected_products.findIndex(
    ...
    CALCULATE_TOTALS(state) {
      ...
      if (state.sale.selected_products.length > 0)
        productTotals = state.sale.selected_products.reduce(
      ...
      state.sale.selected_tickets.forEach(eventTickets => {
        ticketTotals += eventTickets.reduce(
      ...
      let balance =
        total - state.sale.tendered > 0
          ? total - state.sale.tendered - state.sale.paid
          : 0
      let change_due =
        state.sale.tendered - total > 0 ? state.sale.tendered - total : 0
    ...
    SET_EVENT(state, payload) {
      ...
      if (
        state.sale.tickets[payload.index] &&
        state.sale.tickets[payload.index].length > 0
      ) {
        // console.log(state.sale.tickets[payload.index])
        // console.log(state.sale.selected_tickets[payload.index])
        state.sale.selected_tickets[payload.index].forEach(ticket =>
          Object.assign(ticket.event, { id: state.sale.events[payload.index] })
        )
    ...
    SET_SELECTED_TICKETS(state, payload) {
      state.sale.selected_tickets.splice(
        payload.index,
        1,
    ...
    REMOVE_TICKET(state, payload) {
      // Find ticket
      let i = state.sale.tickets[payload.index].findIndex(
        ticket => ticket == payload.id
      )
      let j = state.sale.selected_tickets[payload.index].findIndex(
        ticket => ticket.id == payload.id
      )
      // Reset count
      Object.assign(state.sale.selected_tickets[payload.index][j], {
        amount: 1
      })
      // Remove from array
    ...
    SET_NUMBER_OF_EVENTS(state, payload) {
      if (payload) Object.assign(state, { numberOfEvents: parseInt(payload) })
      else state.numberOfEvents++
    },
