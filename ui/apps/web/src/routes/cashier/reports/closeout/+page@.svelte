	...
	<div>
		...
		{#if data.closeout.report.length > 1}
			<h5 class="text-zinc-500 dark:text-zinc-400">
				Cashiers: {data.closeout.report.map((r) => r.cashier).join(', ')}
			</h5>
		{:else}
			<h5 class="text-zinc-500 dark:text-zinc-400">Cashier: {data.closeout.report[0].cashier}</h5>
		{/if}
	</div>
	<br />
	<table class="table-auto">
		{#each data.closeout.report as report}
			<thead>
				{#if data.closeout.report.length > 1}
					<tr>
