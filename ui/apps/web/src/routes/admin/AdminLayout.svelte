<script lang="ts">
	...
	let { title, header, children, backHref, nav = false }: IAdminLayoutProps = $props();
</script>
...
<article
	class="flex h-[calc(100svh-128px)] flex-col gap-3 overflow-y-auto px-6 pt-6 lg:h-[calc(100svh-64px)]"
>
	{@render children()}
</article>
...
{#if nav}
	<nav
		class="grid auto-cols-auto grid-flow-col gap-1 overflow-x-auto border-t text-xs md:justify-center lg:hidden"
	>
		<a
			href="/admin"
