<script lang="ts">
	...
	let { data } = $props();
	const { version, organization } = data;
</script>

<svelte:head>
	<title>Login | {organization.name} &middot; Astral</title>
</svelte:head>
	...
	<section class="flex w-screen flex-col items-center justify-center gap-3 lg:w-full">
		<article class="grid gap-3">
			...
			<form class="grid gap-6" action="">
				...
				/>
				<a class="ml-auto inline-block text-sm underline" href="/forgot"
					>The email you used to create an account.</a
				>
				<AButton text="Login" />
			</form>
