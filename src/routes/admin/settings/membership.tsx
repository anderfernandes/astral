...
export default function MembershipPage() {
  ...
  const items = createMemo(() => getMembershipTypesFn());

  const selected = createMemo(() =>
    items().find((item) => String(item.id) == location?.query?.id),
  );
  ...
  return (
    <section class="grid gap-3">
      <div>
        <Button
          href={paths.admin.settings.membership({ dialog: "create" })}
        ...
        >
          <form class="grid gap-3" method="post" action={save}>
            ...
            <div class="mt-3 flex justify-end gap-3">
              <Button href="#" text="Cancel" variant="secondary" />
...
const MembershipTypeSchema = v.object({
  name: v.pipe(v.string(), v.minLength(3), v.maxLength(255)),
  description: v.pipe(v.string(), v.minLength(3), v.maxLength(255)),
  //cover: v.file()
  duration: v.pipe(v.number(), v.minValue(0), v.maxValue(365)),
  price: v.pipe(v.number(), v.minValue(0)),
  maxFreeSecondaries: v.pipe(v.number(), v.minValue(0)),
  maxPaidSecondaries: v.pipe(v.number(), v.minValue(0)),
  paidSecondaryPrice: v.pipe(v.number(), v.minValue(0)),
  isActive: v.boolean(),
  isPublic: v.boolean(),
});

const save = action(async (formData: FormData) => {
  "use server";

  const { output, success, issues } = v.safeParse(MembershipTypeSchema, {
  ...
  if (formData.has("id")) {
    const IdSchema = v.pipe(v.number());
    const result = v.safeParse(IdSchema, Number(formData.get("id")));
    ...
    await db.membershipTypes.create({
      ...output,
      creatorId: user.userId as number,
    });
  }
